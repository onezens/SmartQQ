//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "NSObject.h"

@class NSString;

@interface MiniZipArchive : NSObject
{
    void *_zipFile;
    void *_unzFile;
    NSString *_password;
    id _delegate;
}

- (_Bool)CloseZipFile2;
- (_Bool)CreateZipFile2:(id)arg1;
- (_Bool)CreateZipFile2:(id)arg1 password:(id)arg2;
- (void)OutputErrorMessage:(id)arg1;
- (_Bool)OverWrite:(id)arg1;
- (_Bool)QQFilterUnzipFileTo:(id)arg1 overWrite:(_Bool)arg2 fileInfoDic:(id)arg3;
- (_Bool)UnzipCloseFile;
- (_Bool)UnzipFileTo:(id)arg1 overWrite:(_Bool)arg2;
- (_Bool)UnzipFileTo:(id)arg1 overWrite:(_Bool)arg2 fileInfoDic:(id)arg3;
- (_Bool)UnzipFileTo:(id)arg1 overWrite:(_Bool)arg2 firstFile:(id *)arg3;
- (_Bool)UnzipOpenFile:(id)arg1;
- (_Bool)UnzipOpenFile:(id)arg1 Password:(id)arg2;
- (_Bool)addFileStreamToZipNoPwd:(id)arg1 newname:(id)arg2;
- (_Bool)addFileToZip:(id)arg1 newname:(id)arg2;
- (_Bool)addFileToZipWithData:(id)arg1 fileName:(id)arg2;
- (void)dealloc;
@property(retain, nonatomic) id delegate; // @synthesize delegate=_delegate;
- (id)init;
- (void)unZipFailLog:(id)arg1;

@end

