//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "NSObject.h"

#import "QZUploadManagerSendBoxDelegate.h"

@class NSString, QZPhotoPreUploader, QZUploadManager;

@interface QZPhotoUploader : NSObject <QZUploadManagerSendBoxDelegate>
{
    QZUploadManager *_uploadManager;
    QZPhotoPreUploader *_preUploader;
}

+ (id)sharedInstance;
- (void).cxx_destruct;
- (void)addJob:(id)arg1;
- (void)addJobs:(id)arg1;
- (id)createVideoCoverPacketWithVideoJob:(id)arg1 seq:(long long)arg2;
- (void)handleBatchUploadFinished:(id)arg1 batchInfo:(id)arg2;
- (id)initWithUploadManager:(id)arg1 preUploader:(id)arg2;
- (id)saveCoverImage:(id)arg1;
- (void)updateFakeVideoInfo:(id)arg1 videoJob:(id)arg2;
- (void)updateFakefeed:(id)arg1;
- (void)uploadJob:(id)arg1 seq:(long long)arg2;
- (id)uploadManager;
- (void)uploadManager:(id)arg1 beginOperation:(id)arg2 taskID:(long long)arg3 taskDetail:(id)arg4;
- (void)uploadManager:(id)arg1 chanceToModify:(_Bool)arg2 operation:(id)arg3 taskID:(long long)arg4 taskDetail:(id)arg5;
- (void)uploadManager:(id)arg1 fail:(id)arg2 operation:(id)arg3 taskID:(long long)arg4 taskDetail:(id)arg5;
- (void)uploadManager:(id)arg1 finishOperation:(id)arg2 taskID:(long long)arg3 taskDetail:(id)arg4;
- (void)uploadManager:(id)arg1 sucess:(id)arg2 operation:(id)arg3 taskID:(long long)arg4 taskDetail:(id)arg5;
- (void)uploadManager:(id)arg1 syncAlbumName:(id)arg2 operation:(id)arg3 taskID:(long long)arg4 taskDetail:(id)arg5;
- (void)uploadManager:(id)arg1 syncServerTime:(long long)arg2 operation:(id)arg3 taskID:(long long)arg4 taskDetail:(id)arg5;
- (void)uploadManager:(id)arg1 total:(long long)arg2 recv:(long long)arg3 operation:(id)arg4 taskID:(long long)arg5 taskDetail:(id)arg6;
- (void)uploadManager:(id)arg1 updateStatus:(long long)arg2 operation:(id)arg3 taskID:(long long)arg4 taskDetail:(id)arg5;
- (void)uploadVideo:(id)arg1 seq:(long long)arg2;
- (void)videoUploadManager:(id)arg1 fail:(id)arg2 operation:(id)arg3 taskID:(long long)arg4 taskDetail:(id)arg5;
- (void)videoUploadManager:(id)arg1 finishOperation:(id)arg2 taskID:(long long)arg3 taskDetail:(id)arg4;
- (void)videoUploadManager:(id)arg1 total:(long long)arg2 recv:(long long)arg3 operation:(id)arg4 taskID:(long long)arg5 taskDetail:(id)arg6;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

